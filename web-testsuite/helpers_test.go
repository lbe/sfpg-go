...
// ---------------------------------------------------------------------------
// Package State (shared by all test files in this suite)
// ---------------------------------------------------------------------------
...
// in the client's jar.
//
//  1. GET /gallery/1 -> extract CSRF token from the login form
...
func login(t *testing.T, client *http.Client) {
	...
	// Step 1: GET gallery page and extract CSRF token
	...
	if csrfToken == "" {
		t.Fatal("could not extract CSRF token from /gallery/1 login form")
	}
...
func loginRaw(client *http.Client) error {
	// GET gallery page to extract CSRF token
	resp, err := client.Get(serverURL + "/gallery/1")
	if err != nil {
		return fmt.Errorf("GET /gallery/1: %w", err)
	}
	defer resp.Body.Close()

	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return fmt.Errorf("read /gallery/1: %w", err)
	}
...
// skipIfNoIDs skips the test if folderID or fileID is empty (no data in DB).
func skipIfNoIDs(t *testing.T) {
	t.Helper()
	if folderID == "" || fileID == "" {
		t.Skip("SKIP: no folder/file IDs in database")
	}
}
