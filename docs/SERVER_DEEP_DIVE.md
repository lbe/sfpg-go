...
| **`restart.go`**     | Exposes `RestartRequired()` and coordinates process restart via `server.go`: graceful HTTP shutdown followed by `syscall.Exec` of a fresh process image.                                  |
