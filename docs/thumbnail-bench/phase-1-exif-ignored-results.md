...
- `BenchmarkFull_EXIFIgnored` runs the full `GenerateThumbnailAndHashes` path on the committed has-EXIF-thumbnail fixture `testdata/thumbnail/exif-thumb.jpg` with the embedded-thumbnail shortcut disabled, so it measures full decode+resize of the EXIF-bearing source.
...
- **ignored / has-EXIF = 7.36×** (medians); B/op 8.29×.
- Sanity check: forced-ignored (31.7 ms) ≈ real `Full_EXIFMiss` (no EXIF thumbnail; median 32 686 608 ns/op, same file) within run noise — the hook behaves like a genuine no-EXIF-thumbnail full-image decode.
...
RSS (`tmp/thumbnail_rss_parallel_exif_ignored_12mp.txt`, `-benchtime=5s -count=1 -cpu=4`): bench line 185 859 571 ns/op, 92 851 691 B/op, 96 allocs; **Maximum resident set size 505 264 kB ≈ 493.4 MiB**; job CPU 374%. Phase 1 no-EXIF-thumbnail parallel 12 MP RSS was 467 908 kB ≈ 456.9 MiB (+8% this run, same order).

## 7. Decision inputs

- **Does resize still dominate with the EXIF shortcut removed?** Yes. Forced-ignored ≈ real no-EXIF-thumbnail path (31.7 vs 32.7 ms medians). Phase 1 phase-split on the full-image decode path: resize (thumb + pHash) ≈ 82% of phase time, resize-only = 50.7–60.1% of full pipeline at 2/12/25 MP. Removing the shortcut re-opens the full decode+resize cost; resize remains the largest share.
- **Is Phase 2 filter work justified on this path?** Yes. Full-image decode when there is no usable EXIF thumbnail (or when EXIF extract is forced off) is the common real-world case and costs 7.36× the has-EXIF-thumbnail path (31.7 ms vs 4.3 ms); resize is its largest single cost, so filter work targets the dominant phase. Caveat: files with a usable EXIF thumbnail bypass big-image resize entirely (~4.3 ms), so filter gains apply only to full-image decode paths.
