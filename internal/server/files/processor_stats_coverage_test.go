...
func TestProcessingStats_Reset(t *testing.T) {
	...
	t.Run("concurrent reset is safe", func(t *testing.T) {
		...
		if totalFound < 0 {
			t.Errorf("TotalFound negative after concurrent operations: %d", totalFound)
		}
	})
}
...
func TestProcessingStats_ConcurrentAccess(t *testing.T) {
	...
	// Final values should be reasonable (non-negative)
	if stats.TotalFound.Load() < 0 {
		t.Errorf("Final TotalFound negative: %d", stats.TotalFound.Load())
	}
	if stats.AlreadyExisting.Load() < 0 {
		t.Errorf("Final AlreadyExisting negative: %d", stats.AlreadyExisting.Load())
	}
	if stats.NewlyInserted.Load() < 0 {
		t.Errorf("Final NewlyInserted negative: %d", stats.NewlyInserted.Load())
	}
}
