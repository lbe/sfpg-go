...
import (
	...
	"go.local/sfpg/internal/server/middleware"
	"go.local/sfpg/internal/server/pathutil"
	"go.local/sfpg/internal/server/security"
	"go.local/sfpg/internal/server/session"
	"go.local/sfpg/internal/server/template"
...
// addCommonTemplateData adds common template data (auth state, CSRF token, and theme) to template data map
// This is used for pages that include modals which need the CSRF token
// Delegates to template.AddCommonData and adds theme
