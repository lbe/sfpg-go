...
const configKeyExists = `-- name: ConfigKeyExists :one
SELECT EXISTS(SELECT 1 FROM config WHERE key = ?) AS key_exists
`

func (q *Queries) ConfigKeyExists(ctx context.Context, key string) (bool, error) {
	row := q.queryRow(ctx, q.configKeyExistsStmt, configKeyExists, key)
	var key_exists bool
...
func (q *Queries) InsertConfigIfNotExists(ctx context.Context, arg InsertConfigIfNotExistsParams) error {
	_, err := q.exec(ctx, q.insertConfigIfNotExistsStmt, insertConfigIfNotExists,
		arg.Key,
		arg.Value,
		arg.CreatedAt,
		arg.UpdatedAt,
