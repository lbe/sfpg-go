...
func TestConcurrencyLimits(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// Bubble time: 5 tasks, maxConcurrent=2, each takes 100ms, so all tasks
		// complete in ~310ms of fake clock time.
...
func TestOneTimeTaskRunsExactlyOnce(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// Bubble time: wait for the task to complete (50ms) plus a few ticks.
...
func TestContextCancellation(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// Cancel context immediately (bubble time)
		...
		// Shutdown should be timely. With 3 tasks at 200ms each and
		// maxConcurrent=2, the in-flight tasks complete in ~210ms of bubble time.
...
func TestIntervalLogic_DailyRecurring(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// Bubble time: wait for multiple executions
...
func TestOneTimeTask(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// Bubble time: wait for execution
...
func TestTaskAdditionWhileRunning(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// Bubble time: let the scheduler start, then add a task
		time.Sleep(10 * time.Millisecond)
		task := &TestTask{
			id:    "dynamic-task",
			sleep: 10 * time.Millisecond,
...
func TestTaskWithFutureStartTime(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// Bubble time: wait less than futureTime - task should not execute yet
...
func TestShutdownWhileRunning(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// Bubble time: wait a bit to ensure Start is running
		time.Sleep(10 * time.Millisecond)
...
func TestRemoveTask(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// Bubble time: wait for scheduler to check tasks
		time.Sleep(100 * time.Millisecond)
		cancel()
		<-done
...
func TestRemoveTask_ExecutingTask(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// Bubble time: wait a bit for the task to start executing (spawned at the
		// first 10ms tick, running until ~210ms).
		time.Sleep(50 * time.Millisecond)
...
func TestStart_AlreadyRunning(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// Wait until the scheduler goroutine has set running and parked.
		synctest.Wait()
...
func TestStart_TimerDrivenPoll(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// Bubble time: the task fires at +50ms and completes ~10ms later.
		time.Sleep(100 * time.Millisecond)
		cancel()
		<-done
...
func TestStart_WakeChannelDrainsTimer(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// Bubble time: let the scheduler run its first (empty) tick and settle
		// into an idle timer wait before adding work.
		time.Sleep(20 * time.Millisecond)
...
func TestStart_ShutdownContextPath(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// Bubble time: wait for the task to start executing.
...
func TestRemoveTask_RecurringTask(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// Bubble time: wait for the first execution(s)
		time.Sleep(50 * time.Millisecond)
		...
		// Remove task; an execution already in flight when the removal lands may
		// still complete, so the count can legitimately grow by one after removal.
		err = scheduler.RemoveTask(id)
		if err != nil {
			t.Fatalf("Failed to remove task: %v", err)
		}

		// Bubble time: let any in-flight execution finish, then verify the count
		// stabilizes (no further executions are scheduled after removal).
		time.Sleep(100 * time.Millisecond)
		settledCount := task.ExecutionCount()
		...
		if finalCount != settledCount {
			t.Errorf("Task should not execute again after removal. Settled count: %d, final count: %d", settledCount, finalCount)
		}
	})
}
